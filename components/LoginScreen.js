...
const saveUserData = async (token, _id, name) => {
  try {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        const { token, _id, name } = responseData; // Gelen yanıtın içeriğine göre düzenleyin
        await saveUserData(token, _id, name); // Token, ID ve name bilgisini kaydet
