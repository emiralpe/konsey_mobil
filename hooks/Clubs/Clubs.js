...
import barulogo from '../../assets/barulogo.png';
...
// İlgini çekebilecek kulüpleri getiren fonksiyon
const fetchOtherClubs = async (setOtherClubs) => {
  try {
    const userId = await AsyncStorage.getItem('userId'); // Kullanıcı ID'sini al
    const token = await AsyncStorage.getItem('userToken'); // Kullanıcı Token'ını al
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error fetching other clubs:', data.message);
      }
    } catch (jsonError) {
      console.error('JSON Parse Error:', jsonError);
    }
  ...
  return (
    <View>
      ...
      <View className='flex-row mt-10 items-center justify-between'>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("AllClubs")}>
          <Text className='text-[#2AB8E7] text-xl mr-5 font-[Bold]'>Tümü</Text>
        </TouchableOpacity>
      </View>
      <View className='items-center'>
        {userClubs.length === 0 ? (
          <Text className='text-[#2AB8E7] text-xl font-[Semibold] mt-10'>Üye olduğunuz kulüp bulunmamaktadır.</Text>
        ) : (
          userClubs.slice(0, 3).map((club, index) => (
            <TouchableOpacity key={index} onPress={() => navigation.navigate("ClubDetails", { clubId: club._id })}>
              <View className='w-[90%] col-span-12 shadow items-center flex-row rounded-xl mt-4 h-28 bg-[#fff]'>
                <Image className=' ml-2 col-span-3 rounded-full w-24 h-24' source={barulogo} />
      ...
      <View className='flex-row mt-10 items-center justify-between'>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("AllOtherClubs")}>
          <Text className='text-[#2AB8E7] text-xl mr-5 font-[Bold]'>Tümü</Text>
        </TouchableOpacity>
      </View>
      <View>
        <View className='items-center'>
          {otherClubs.length > 0 ? (
            ...
            <Text className='text-center text-base mt-4'>İlginizi çekecek kulüp bulunamadı.</Text>
          )}
        </View>
      </View>
    </View>
