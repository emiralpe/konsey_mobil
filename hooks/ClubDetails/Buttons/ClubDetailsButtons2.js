    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/clubs/post/contact', {
        ...
        body: JSON.stringify({
          ...
          subject: subject.value || subject, // subject.value yoksa subject kullan
          title,
          message,
          status: true,
  ...
  const handleApply = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error applying to club:', responseText);
        Alert.alert('Hata', `${responseText}`);
      }
    } catch (error) {
      ...
      >
        <View className='flex-1 bg-white bg-opacity-50'>
          <View className='bg-white p-6 rounded-lg w-full mt-20'>
            ...
            <SelectList
  setSelected={(val) => {
    const selectedOption = subjectOptions.find(option => option.key === val);
