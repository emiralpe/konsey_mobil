...
export default function OfferDetails({ route }) {
  ...
  useEffect(() => {
    const fetchOfferDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/student-preferences/get/offer/${offerId}`, {
          method: 'GET',
          headers: {
  ...
  const fetchCode = async () => {
    try {
      ...
      console.log('Fetching code for offer ID:', offerId);
      const response = await fetch('http://localhost:3000/api/student-preferences/post/offer/code', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        if (errorData.code) {
          ...
          setGeneratedCode(errorData.code);
        ...
        console.log('Fetched code ID:', codeId);
  ...
  const fetchGeneratedCode = async (codeId) => {
    try {
      ...
      console.log('Fetching generated code for code ID:', codeId);
      const response = await fetch(`http://localhost:3000/api/student-preferences/get/offer/code/${codeId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      const data = await response.json();
      console.log('Fetched generated code:', data);
      if (data.code) {
        setGeneratedCode(data.code); // Adjust this line based on the actual response structure
        setModalVisible(true);
      } else {
        Alert.alert(
          "Hata!",
